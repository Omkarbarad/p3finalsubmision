...
export default function Card({ element }) {
  ...
  const generateUniqueKey = (element) => {
    return element.id || `${element.songname}-${element.albumname}`;  // Improved unique key generation
  };
  ...
  return (
    <>
      <div key={uniqueKey} className="col-lg-3 col-md-6 col-sm-12 py-2 d-flex align-items-stretch">
        <div className="card bg-dark text-white shadow-lg d-flex flex-column">
          ...
          <div className="card-body d-flex flex-column">
            ...
            <div className="audio-controls mt-auto">
              <audio src={`http://localhost:3000${element.url}`} controls className="w-100"></audio>
            </div>
          </div>
        </div>
      </div>
